...
module.exports = {
  ...
  create: function (req, res, next) {
    ...
    Variant.create(carData).exec(function (err, variant) {
      if (err) {
  ...
  deleteAll: function (req, res, next) {
    Variant.destroy().exec(function (err) {
      if (err) return next(err);
      return res.send();
    });
  },
  search: function (req, res, next) {
    ...
    Variant.find(query).exec(function (err, variant) {
      if (err) return res.badRequest(err);
      if (variant.length > 0) return res.json(variant);
      else return res.notFound(variant);
    });
  },
};
