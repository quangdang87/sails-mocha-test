...
Promise.all([readUnitXML, readCloverXML, readlintXML])
  .then((data) => {
...
function getCloverCount(cloverJson) {
  ...
  let uncoveredConditions =
    parseInt(metrics._attributes.conditionals) -
    parseInt(metrics._attributes.coveredconditionals);
  let uncoveredMethods =
    parseInt(metrics._attributes.methods) -
    parseInt(metrics._attributes.coveredmethods);
  let uncovered_count =
    uncoveredStatements + uncoveredMethods + uncoveredConditions;
...
function getUnitCount(unitJson, cloverResults, lintResults) {
  ...
  unitJsonResult.testsuites.testsuite[
    testsuite_count_unit
  ]._attributes.id = testsuite_count_unit;
  unitJsonResult.testsuites.testsuite[
    testsuite_count_unit
  ]._attributes.tests = totalUnitClover;
  unitJsonResult.testsuites.testsuite[
    testsuite_count_unit
  ]._attributes.failures = totalfailure;

  if (failurecloverCount > 0) {
    for (var i = 0; i < failurecloverCount; i++) {
      unitJsonResult.testsuites.testsuite[testsuite_count_unit].testcase.push(
        FAILURE_TO_WRITE
      );
    }
  }
